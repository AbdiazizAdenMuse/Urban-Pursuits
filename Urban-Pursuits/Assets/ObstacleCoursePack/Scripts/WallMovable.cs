...
public class WallMovable : MonoBehaviour
{
    ...
    public float moveUpAmount = 1f; // Increased amount the wall moves upwards
    ...
    void Update()
    {
        if (isDown)
        {
            ...
            {
                transform.position += Vector3.up * Time.deltaTime * speed * moveUpAmount; // Adjusting the distance moved upwards
            }
            else if (!isWaiting)
